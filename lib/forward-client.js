...
const net = require('net'),
      ...
      Rx = require('rxjs/Rx'),
      {Msg, Reader, deserialize, frame, DEFAULT_MAX_MESSAGE_LEN} = require('./proto');

const emptyBuffer = new Buffer(0);

class ForwardClient {
    constructor(logger, host, port = 5555, minFlushBufferSize = 1000, maxBufferSize = 2000, maxFlushInterval = 1000, reconnectTimeout = 1000, flushTimeout = 60000) {
        this.logger = logger;
        this.host = host;
        this.port = port;
        this.client = null;
    ...
    enqueue(messages) {
        ...
        for(const message of messages) {
            if (this.bufferWriteIndex + message.length > this.maxBufferSize){
                ...
                this.logger.error('Riemann client buffer full, dropping ' + dropped +
                    ' message(s), last message evaluated was of ' + message.length + ' bytes');
                break;
            }
            consumedMessages += 1;
