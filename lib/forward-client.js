...
class ForwardClient {
    ...
    this.client = net
      ...
      .on("data", (data) => {
        ...
        for (const raw of messages) {
          ...
          logger.debug(`Server responded with ${JSON.stringify(ack)}`);
          if (ack.ok) {
            if (this.flushHandler !== null) {
              this.flushHandler.resolve();
            }
          } else {
            const error = `Server returned error in response: ${ack.error}`;
            logger.error(error);
  ...
  checkFlushTimeout() {
    ...
    if (this.flushHandler !== null && timeSinceLastMessage > this.flushTimeout) {
      const error = `Server did not respond for ${timeSinceLastMessage}ms`;
      logger.error(error);
      this.handleRemoteError(new Error(error));
    }
  }
