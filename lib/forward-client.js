...
const { Reader, deserialize, frame, DEFAULT_MAX_MESSAGE_LEN } = require("./proto");
...
class ForwardClient {
  constructor(
    ...
    this.reader = new Reader(DEFAULT_MAX_MESSAGE_LEN); // messages are only acks, so no need to configure
    ...
    logger.debug(`Starting forward client from ${host}:${port}`);
  }
  ...
  connect() {
    this.client = net
      .createConnection({ host: this.host, port: this.port }, () => {
        logger.info(`Riemann client connected to: ${this.host}:${this.port}`);
      })
      .on("end", () => {
        logger.warn(`Riemann client disconnected, attempting reconnect in ${this.reconnectTimeout}ms`);
      ...
      .on("data", (data) => {
        ...
        for (const raw of messages) {
          ...
          logger.debug(`Riemann server responded with ${ack}`);
          if (ack.ok) {
            if (this.flushHandler !== null) {
      ...
      .on("error", (error) => {
        logger.error(`Riemann client error: ${error} attempting reconnect in ${this.reconnectTimeout}ms`);
  ...
  enqueue(messages) {
    ...
    for (const message of messages) {
      if (this.bufferWriteIndex + message.length > this.maxBufferSize) {
        ...
        logger.error("Riemann client buffer full, dropping " + dropped + " message(s)");
        break;
      }
      consumedMessages += 1;
  ...
  close() {
    logger.info("Closing remote forwarder");
    this.isClosed = true;
    try {
      this.client.end();
    } catch (e) {
      logger.error("Could not close forwarding client", e);
    }
  }
}
