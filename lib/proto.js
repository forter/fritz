...
const serialize = (msg) => frame(Msg.encode(msg).finish());

const unframe = (framed) => framed.slice(INT_BYTES_LEN);

const deserialize = (framed) => Msg.decode(unframe(framed));
...
export { DEFAULT_MAX_MESSAGE_LEN, deserialize, Event, frame, Msg, Reader, serialize, unframe };
